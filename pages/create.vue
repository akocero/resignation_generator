            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <button
                    @click="resignationStore.setLanguage('english')"
                    :class="[
                      'py-2 px-4 rounded-lg font-medium transition-all border-2',
                      resignationStore.language === 'english'
                  ...
                  <button
                    @click="resignationStore.setLanguage('tagalog')"
                    :class="[
                      'py-2 px-4 rounded-lg font-medium transition-all border-2',
                      resignationStore.language === 'tagalog'
                        ? 'bg-indigo-600 text-white border-indigo-600'
                        : 'bg-white text-gray-700 border-gray-300 hover:border-indigo-300',
                    ]"
                  >
                ...
                <p class="text-xs text-gray-500 mt-2">
                  The resignation letter will be generated in this language
                </p>
              </div>
        ...
        <div class="lg:col-span-2">
          <div class="bg-white rounded-2xl shadow-lg p-8 md:p-12">
            <div class="max-w-3xl mx-auto">
              ...
              >
                <!-- Date -->
                <p class="text-right text-gray-600">{{ currentDate }}</p>

                <!-- Manager Address -->
                <div>
                  <p class="font-semibold">
                    {{ formData.managerName || "[Manager's Name]" }}
                  </p>
                  <p>{{ formData.companyName || "[Company Name]" }}</p>
                </div>

                <!-- Salutation -->
                <p>Dear {{ formData.managerName || "[Manager's Name]" }},</p>
...
const generateWithAI = async () => {
  try {
    ...
    if (response.success && response.letter) {
      ...
      alert("✨ AI-generated letter created successfully! Check the preview.");
    }
  } catch (error) {
...
const generatePDF = async () => {
  ...
  try {
    ...
    yPosition += 6;
    doc.setFont("helvetica", "normal");
    doc.text(formData.value.companyName || "[Company Name]", margin, yPosition);
    yPosition += 10;
    ...
    yPosition += 6;
    doc.setFont("helvetica", "normal");
    doc.text(formData.value.position || "[Your Position]", margin, yPosition);
